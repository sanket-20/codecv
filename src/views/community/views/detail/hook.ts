...
import useUserStore, { initialInfo } from '@/store/modules/user';
...
export function useArticleDetail(articleId: Ref<number>, posterCommentId: Ref<number>) {
  ...
  async function like(clicked: boolean) {
    if (!isLogin()) return errorMessage('Please log in first');
    if (clicked) {
      warningMessage('You have already liked this, no need to click again~');
      return;
    }
  ...
  watchEffect(async () => {
    ...
    // Query data and return the specific comment position
...
export function useDelayMenuBar(articleId: Ref) {
  // Temporary solution for asynchronous rendering
